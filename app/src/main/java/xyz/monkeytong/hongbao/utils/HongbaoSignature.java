...
public class HongbaoSignature {
    ...
    public boolean generateSignature(AccessibilityNodeInfo node, String excludeWords, long time) {
        this.nowtime = time;
    ...
    public boolean generateSignature(AccessibilityNodeInfo node, String excludeWords) {
        try {
            ...
            Log.d(TAG, "generateSignature: hongbaoNode.getClassName()=" + hongbaoNode.getClassName());
            ...
            Log.d(TAG, "generateSignature: hongbaoContent=" + (hongbaoContent == null ? "null" : hongbaoContent));
            ...
            String s = this.getSignature(hongbaoInfo[0], hongbaoContent, hongbaoInfo[1]);
            Log.d(TAG, "generateSignature: s==" + s + "   " + this.toString());
            for (int i = 0; i <hongbaoInfo.length ; i++) {
                Log.d(TAG, "generateSignature: hongbaoInfo "+i+" ="+hongbaoInfo[i]);
            }
            if (s.equals(this.toString()) && (lasttime == nowtime)) {
                lasttime = nowtime;
                return false;
            }
