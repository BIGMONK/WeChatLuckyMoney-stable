...
public class HongbaoService extends AccessibilityService implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    public void onAccessibilityEvent(AccessibilityEvent event) {
        ...
        /* 检测通知消息 */
        if (!mMutex) {
            boolean w = watchNotifications(event);
            if (sharedPreferences.getBoolean("pref_watch_notification", false) && w) {
                Log.d(TAG, "onAccessibilityEvent: watchNotifications=" + w);
                return;
            }
    ...
    private void watchChat(AccessibilityEvent event) {
        ...
        Log.d(TAG, "watchChat: mLuckyMoneyReceived=" + mLuckyMoneyReceived
                ...
                + "  (mReceiveNode != null)=" + (mReceiveNode != null));
        /* 如果已经接收到红包并且还没有戳开 */
        ...
        if (mUnpackCount == 1 && (mUnpackNode != null)) {
            double exTime = random.nextInt(100) * (Math.pow(-1, random.nextInt(100)));
            if (exTime < -80) {
                exTime = Math.abs(exTime);
            }
            int delayFlag = sharedPreferences.getInt("pref_open_delay", 0) * 100;
            if (delayFlag != 0)
                delayFlag += +(int) exTime;
            Toast.makeText(getApplicationContext(), "延迟" + delayFlag + "毫秒", Toast.LENGTH_SHORT).show();
    ...
    private void setCurrentActivityName(AccessibilityEvent event) {
        Log.d(TAG, "setCurrentActivityName: ");
    ...
    private boolean watchList(AccessibilityEvent event) {
        ...
        Log.e(TAG, "watchList: " + "nodes=" + (nodes == null ? "null" : (nodes.size())));
    ...
    private void checkNodeInfo(int eventType) {
        Log.d(TAG, "checkNodeInfo: ");
        if (this.rootNodeInfo == null) return;

        Log.d(TAG, "checkNodeInfo: 1");
        ...
        AccessibilityNodeInfo node1 = (sharedPreferences.getBoolean("pref_watch_self", false)) ?
                this.getTheLastNode(WECHAT_VIEW_OTHERS_CH
                        , WECHAT_VIEW_SELF_CH //自己发的红包显示查看红包
                ) : this.getTheLastNode(WECHAT_VIEW_OTHERS_CH);

        Log.d(TAG, "checkNodeInfo: 2" + (node1 != null) + "   " + currentActivityName);
        ...
        if (node1 != null &&
            ...
            Log.d(TAG, "checkNodeInfo:generateSignature ");
            boolean bb = this.signature.generateSignature(node1, excludeWords, System.currentTimeMillis());
            Log.d(TAG, "checkNodeInfo: ###bbbb==" + bb);
            if (bb) {
                mLuckyMoneyReceived = true;
                mReceiveNode = node1;
                Log.d(TAG, "sig" + this.signature.toString());
            }
            return;
        }
